...
Run with:  uv run python -m state_access.collect
"""
...
def main() -> None:
    ...
    state.to_parquet(DATA_DIR / "hot_cold_state.parquet", index=False)
    tradeoff.to_parquet(DATA_DIR / "tradeoff.parquet", index=False)
    gas.to_parquet(DATA_DIR / "gas_concentration.parquet", index=False)
    (DATA_DIR / "totals.json").write_text(json.dumps(totals, indent=2))

    print(f"\nWrote 3 parquets + totals.json to {DATA_DIR}")
