...
One aggregation query (no backfill). Run with:  uv run python -m state_access.v1.collect_gas
"""
...
def main() -> None:
    ...
    df.to_parquet(DATA_DIR_V1 / "gas_daily.parquet", index=False)

    print(f"Wrote {len(df)} daily gas rows to {DATA_DIR_V1 / 'gas_daily.parquet'}")
