...
uv run python -m state_access.v1.collect     # queries ClickHouse → data/v1/*.parquet + totals.json
uv run python -m state_access.v1.analysis     # data/ → 4 PNG charts + printed tables
```
...
checkpointing `data/v1/history_w30.parquet` after each anchor (resumable).

```bash
uv run python -m state_access.v1.collect_history   # ~1–1.5 hr, resumable
uv run python -m state_access.v1.analysis_history   # 3 trend charts
```

Outputs: `data/v1/history_w30.parquet` and `history_{state_cold,writes_cold,gas_concentration}.png`.
