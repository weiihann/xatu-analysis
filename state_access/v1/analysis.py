...
a script, saving PNGs next to the parquets.
    ...
    uv run python -m state_access.v1.analysis
"""
...
state = pd.read_parquet(DATA_DIR_V1 / "hot_cold_state.parquet")
tradeoff = pd.read_parquet(DATA_DIR_V1 / "tradeoff.parquet")
gas = pd.read_parquet(DATA_DIR_V1 / "gas_concentration.parquet")
totals = json.loads((DATA_DIR_V1 / "totals.json").read_text())
...
fig1.write_image(DATA_DIR_V1 / "hot_share_vs_window.png", scale=2)
show(fig1)
...
fig2.write_image(DATA_DIR_V1 / "cold_share_vs_window.png", scale=2)
show(fig2)
...
fig3.write_image(DATA_DIR_V1 / "tradeoff_cold_vs_writes.png", scale=2)
show(fig3)
...
fig4.write_image(DATA_DIR_V1 / "gas_concentration.png", scale=2)
show(fig4)
