...
uv run python -m state_access.v2.collect          # slot/account/slot_typed histograms (~20 min; resumable)
uv run python -m state_access.v2.collect_history  # full-history event totals (~10 min; resumable)
uv run python -m state_access.v2.collect_sweep    # Part III weekly sweep, T={30,90,180,365} (DAYS; resumable per (T,anchor))
uv run python -m state_access.v2.analysis         # derives Parts I/II tables + charts (no DB; ~1 min, but kaleido is slow)
uv run python -m state_access.v2.analysis_sweep   # Part III verification + time-series charts (no DB)
```
...
- **`slot_first_op`, `account_first_op`, `account_r_empty_split`** → run by
  `_sweep_resume_all.py` (`uv run python -m state_access.v2.sweep_resume_all`, idempotent —
